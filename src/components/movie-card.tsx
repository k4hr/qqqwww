...
export function MovieCard({ movie }: Props) {
  return (
    <article className="mf-card group relative block">
      <Link href={`/movie/${movie.slug}`} aria-label={`Подробнее: ${movie.titleRu}`} className="absolute inset-0 z-10" />
      <div className="poster-fallback relative aspect-[2/3] overflow-hidden">
        ...
        <span className="mf-badge absolute left-3 top-3 z-20">{movie.quality || "HD"}</span>
        <div className="absolute bottom-3 right-3 z-20 rounded-full border border-white/10 bg-black/60 px-2.5 py-1 text-[11px] font-black text-white backdrop-blur">{movie.year}</div>
        <div className="pointer-events-none absolute inset-0 z-20 flex items-center justify-center bg-[radial-gradient(circle_at_center,rgba(229,9,20,.24),rgba(0,0,0,.54))] opacity-0 transition-opacity duration-300 group-hover:opacity-100">
          <Link href={`/watch/${movie.slug}`} className="pointer-events-auto flex items-center gap-2 rounded-full bg-[#e50914] px-4 py-2 text-xs font-black text-white shadow-[0_0_32px_rgba(229,9,20,.42)] transition-transform hover:scale-105">
