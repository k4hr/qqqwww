...
export function MovieHeroSlider({ movies }: { movies: HeroMovie[] }) {
  ...
  if (!slideCount) {
    return (
      <section className="hero-slider poster-fallback relative overflow-hidden rounded-[30px] border border-white/10">
        ...
        <div className="relative z-10 flex min-h-[460px] max-w-2xl flex-col justify-end p-5 sm:min-h-[560px] sm:p-12">
          <h1 className="text-[clamp(2rem,10vw,3.75rem)] font-black tracking-[-.045em] text-white">REDFILM</h1>
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 grid min-h-[500px] items-end gap-7 p-5 sm:min-h-[560px] sm:p-8 md:min-h-[620px] md:grid-cols-[minmax(0,1fr)_240px] md:items-center lg:grid-cols-[minmax(0,1fr)_300px] lg:p-14">
        <div className="max-w-3xl pb-14 md:pb-0">
          ...
          <h1 className="break-words text-[clamp(2rem,9vw,4.5rem)] font-black leading-[.98] tracking-[-.05em] text-white drop-shadow-2xl">{movie.titleRu}</h1>
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            <Link href={`/watch/${movie.slug}`} className="mf-btn mf-btn-primary gap-2 max-[420px]:w-full"><Play size={16} fill="currentColor" /> Смотреть</Link>
            <Link href={`/movie/${movie.slug}`} className="mf-btn gap-2 max-[420px]:flex-1"><Info size={16} /> Подробнее</Link>
            <Link href={`/similar/${movie.slug}`} className="mf-btn max-[420px]:flex-1">Похожие</Link>
          </div>
        </div>
