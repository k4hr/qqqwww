...
export function LibraryMovieCard({ movie, onRemove }: { movie: ClientMovie; onRemove?: (id: string) => void }) {
  return <article className="mf-card group relative min-w-0">
    ...
    <div className="poster-fallback relative aspect-[2/3] overflow-hidden">
      {movie.posterUrl ? <Image src={movie.posterUrl} alt={movie.title} fill loading="lazy" className="object-cover transition duration-500 group-hover:scale-[1.04]" sizes="(max-width: 640px) calc(50vw - 20px), 180px" quality={68} /> : <div className="absolute inset-0 flex items-center justify-center text-[#5d5d67]"><Film size={38} /></div>}
