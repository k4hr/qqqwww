...
export function SearchAutocomplete({ mobile = false }: { mobile?: boolean }) {
  ...
  return <div ref={rootRef} className={shellClass}>
    <form onSubmit={submitSearch} action="/search" className="flex h-11 w-full items-center rounded-[11px] border border-white/[.07] bg-white/[.025] px-3.5 transition focus-within:border-white/[.16] focus-within:bg-white/[.04]">
      ...
      <button type="submit" aria-label="Найти" className="ml-2 inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-[#a0a1a8] transition hover:bg-white/[.05] hover:text-white"><Search size={18} /></button>
    </form>
    {open && (loading || results.length > 0) ? <div className="absolute inset-x-0 top-[calc(100%+8px)] z-[90] max-h-[min(420px,60vh)] overflow-y-auto rounded-[13px] border border-white/[.07] bg-[#090a0c]/[.98] p-1.5 shadow-[0_24px_70px_rgba(0,0,0,.55)] backdrop-blur-xl">
      {loading ? Array.from({ length: 3 }, (_, index) => <div key={index} className="grid min-h-14 grid-cols-[38px_minmax(0,1fr)] items-center gap-3 p-2"><div className="skeleton h-12 rounded-md" /><div className="space-y-2"><div className="skeleton h-3 w-4/5 rounded" /><div className="skeleton h-3 w-2/5 rounded" /></div></div>) : results.map((movie) => <Link key={movie.id} href={movie.href} onClick={() => setOpen(false)} className="grid min-h-14 grid-cols-[38px_minmax(0,1fr)] items-center gap-3 rounded-[9px] p-2 transition hover:bg-white/[.04]">
        ...
        <div className="min-w-0"><div className="truncate text-sm font-medium text-[#ededf0]">{movie.title}</div><div className="mt-1 text-xs text-[#8d8d97]">{movie.year} · {movie.type === "SERIES" ? "Сериал" : movie.type === "ANIME" ? "Аниме" : movie.type === "CARTOON" ? "Мультфильм" : "Фильм"}</div></div>
      </Link>)}
    </div> : null}
  </div>;
}
