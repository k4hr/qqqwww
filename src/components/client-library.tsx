...
export function ClientLibrary({ mode }: { mode: Mode }) {
  ...
  if (!shown.length) return <div className="border-y border-white/[.07] py-10 text-center"><p className="text-[#a1a1aa]">{emptyText}</p><Link href="/films" className="mf-btn mf-btn-primary mt-5">Перейти к фильмам</Link></div>;

  if (mode === "recent-home") return <section className="rf-section overflow-hidden"><div className="rf-section-header"><h2 className="rf-section-title">Недавно смотрели</h2><Link href="/history" className="rf-section-link">Все →</Link></div><div className="movie-grid home-movie-strip">{shown.map((movie) => <LibraryMovieCard key={movie.id} movie={movie} />)}</div></section>;
