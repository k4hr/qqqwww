...
export default async function CollectionsPage({ searchParams }: Props) {
  ...
  const hubs = current === "bloggers" ? await prisma.creatorHub.findMany({
    where: { isPublished: true },
    orderBy: { updatedAt: "desc" },
  }) : [];
  const partnerIds = hubs.map((hub) => hub.partnerId);
  const [partners, collectionCounts] = current === "bloggers" ? await Promise.all([
    prisma.partner.findMany({ where: { id: { in: partnerIds }, status: "ACTIVE" } }),
    prisma.creatorCollection.groupBy({ where: { partnerId: { in: partnerIds }, status: "PUBLISHED" }, by: ["partnerId"], _count: { _all: true } }),
  ]) : [[], []];
  ...
  return (
    <div className="container py-6">
      <h1 className="text-3xl font-black tracking-[-.035em] text-white">Подборки фильмов и сериалов</h1>
      <div className="mt-5 inline-flex rounded-2xl border border-white/10 bg-white/[0.03] p-1">
        <Link href="/collections?view=bloggers" className={`rounded-xl px-4 py-2 text-sm font-black ${current === "bloggers" ? "bg-[#e50914] text-white" : "text-white/70"}`}>Подборки блогеров</Link>
        <Link href="/collections?view=redfilm" className={`rounded-xl px-4 py-2 text-sm font-black ${current === "redfilm" ? "bg-[#e50914] text-white" : "text-white/70"}`}>Подборки REDFILM</Link>
      </div>
      ...
      {current === "bloggers" ? (
        <div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {hubs.map((hub) => {
            ...
            <Link key={hub.id} href={`/collections/${hub.slug}`} className="mf-panel group overflow-hidden p-0 hover:border-[#e50914]/50">
              <div className="relative h-44 bg-white/[0.04]">
                {hub.coverUrl ? <Image src={hub.coverUrl} alt={hub.title} fill className="object-cover transition-transform group-hover:scale-[1.02]" unoptimized /> : null}
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/10 to-transparent" />
                {partner.avatarUrl ? <Image src={partner.avatarUrl} alt={partner.publicName || partner.name} width={72} height={72} className="absolute bottom-4 left-4 h-18 w-18 rounded-full border-2 border-white object-cover" unoptimized /> : null}
              </div>
              <div className="p-5">
          ...
          {!hubs.length ? <div className="mf-panel p-5 text-[#a1a1aa]">Опубликованных подборок блогеров пока нет.</div> : null}
        </div>
      ) : (
