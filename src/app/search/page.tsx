    ...
    <div className="container py-6">
      ...
      <section className="glass-panel section-glow mb-6 rounded-[24px] p-5 sm:p-6">
        ...
        <form className="mt-5 grid gap-2 md:grid-cols-[minmax(220px,1fr)_150px_130px_180px_auto]" action="/search">
          ...
          <input name="q" defaultValue={query} className="h-12 min-w-0 rounded-2xl border border-white/10 bg-black/30 px-4 text-white outline-none placeholder:text-[#666670] focus:border-[#e50914]" placeholder="Название, оригинальное название или ID" />
          <select name="type" defaultValue={params.type ?? ""} className="h-12 min-w-0 rounded-2xl border border-white/10 bg-[#111118] px-3 text-white"><option value="">Все типы</option><option value={ContentType.MOVIE}>Фильмы</option><option value={ContentType.SERIES}>Сериалы</option></select>
          <select name="year" defaultValue={params.year ?? ""} className="h-12 min-w-0 rounded-2xl border border-white/10 bg-[#111118] px-3 text-white"><option value="">Все годы</option>{Array.from({ length: 20 }, (_, index) => currentYear - index).map((year) => <option key={year} value={year}>{year}</option>)}</select>
          <select name="genre" defaultValue={params.genre ?? ""} className="h-12 min-w-0 rounded-2xl border border-white/10 bg-[#111118] px-3 text-white"><option value="">Все жанры</option>{genres.map((genre) => <option key={genre.id} value={genre.slug}>{genre.name}</option>)}</select>
