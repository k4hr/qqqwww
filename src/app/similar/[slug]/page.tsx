...
export default async function SimilarPage({ params }: Props) {
  ...
  return <div className="container py-6">
    ...
    <section className="mf-panel mt-6 p-5 sm:p-6"><h2 className="text-xl font-black text-white">Что ещё посмотреть</h2><div className="mt-4 flex flex-wrap gap-2"><Link href={likePath(movie)} className="mf-btn">Что посмотреть после</Link>{movie.genres.slice(0, 4).map((item) => <Link key={item.genreId} href={genrePath(item.genre)} className="mf-btn">{item.genre.name}</Link>)}<Link href={yearPath(movie)} className="mf-btn">Фильмы {movie.year} года</Link></div></section>
  </div>;
}
