...
import { countryPath, genrePath, likePath, similarPath, siteUrl, watchPath, yearPath } from "@/lib/seo-links";
...
export default async function SimilarPage({ params }: Props) {
  ...
  return <div className="container py-6">
    ...
    <section className="mf-panel p-4 sm:p-6"><h2 className="mb-5 text-2xl font-black text-white">Почему эти фильмы похожи</h2><div className="space-y-4">{similar.map((item, index) => <article key={item.id} className="grid grid-cols-[72px_minmax(0,1fr)] gap-3 border-b border-white/10 pb-4 last:border-0 sm:grid-cols-[90px_minmax(0,1fr)]"><Link href={watchPath(item)} className="poster-fallback relative aspect-[2/3] overflow-hidden rounded-xl">{item.posterUrl ? <Image src={item.posterUrl} alt={item.titleRu} fill className="object-cover" sizes="90px" unoptimized /> : null}</Link><div className="min-w-0"><h3 className="text-lg font-black text-white">{index + 1}. <Link href={watchPath(item)}>{item.titleRu} ({item.year})</Link></h3><p className="line-clamp-2 mt-2 text-sm text-[#a9a9b2]">{item.description}</p><p className="mt-2 text-sm text-[#d0d0d6]"><b>Почему похож:</b> {item.similarityReasons.join("; ")}.</p><div className="mt-3 flex flex-wrap gap-2"><Link href={watchPath(item)} className="mf-btn mf-btn-primary">Смотреть</Link></div></div></article>)}</div></section>
  </div>;
}
