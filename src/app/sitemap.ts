...
import { filmPath, likePath, similarPath, siteUrl, watchPath } from "@/lib/seo-links";
...
async function buildSitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const paths = [
    ...staticPaths,
    ...movies.flatMap((movie) => [filmPath(movie), watchPath(movie), similarPath(movie), likePath(movie)]),
  ...
  return Array.from(new Set(paths)).map((path) => {
    const rawMovieSlug = path.match(/^\/(?:film|watch|similar|like)\/(.+)$/)?.[1];
    const movieSlug = path.startsWith("/film/") && rawMovieSlug?.endsWith("-smotret-online")
      ? rawMovieSlug.slice(0, -"-smotret-online".length)
      : rawMovieSlug;
    return { url: siteUrl(path), lastModified: movieSlug ? updatedBySlug.get(movieSlug) ?? new Date() : new Date() };
  });
}

const getCachedSitemap = unstable_cache(buildSitemap, ["redfilm-seo-sitemap-v1"], { revalidate: 3600 });
