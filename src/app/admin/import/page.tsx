...
export default async function ImportPage({ searchParams }: Props) {
  ...
  return (
    <div className="container admin-shell py-6">
      ...
      <h1 className="text-3xl font-bold mt-3 mb-2 text-[#222]">Импорт карточек</h1>
      ...
      <section className="vip-panel p-5 mb-6">
        ...
        <form className="grid md:grid-cols-[1fr_180px_180px_150px] gap-3" action="/admin/import">
          ...
          <select name="source" defaultValue={source} className="rounded-xl border border-[#ddd] bg-white text-[#222] h-12 px-4 outline-none focus:border-[#e50914]">
            <option value="kinopoisk">Kinopoisk Unofficial</option>
            <option value="tmdb">TMDB</option>
          </select>
          <select name="type" defaultValue={type} className="rounded-xl border border-[#ddd] bg-white text-[#222] h-12 px-4 outline-none focus:border-[#e50914]">
            <option value="MOVIE">Фильм</option>
            <option value="SERIES">Сериал</option>
            <option value="CARTOON">Мультфильм</option>
            <option value="ANIME">Аниме</option>
          </select>
          <button className="rounded-xl bg-gradient-to-r from-[#f7e2a9] via-[#c9a86a] to-[#8a6d3a] text-[#0b1020] font-bold h-12" type="submit">Найти</button>
        </form>
      </section>
            ...
            {results.map((item) => (
              <div key={`${item.source}-${item.id}`} className="vip-soft-panel p-4 flex gap-4">
                ...
                <div className="min-w-0 flex-1">
                  <div className="font-bold leading-tight line-clamp-2 text-[#222]">{item.title}</div>
                  <div className="text-sm text-neutral-500 mt-1 line-clamp-1">{item.originalTitle}</div>
                  <div className="text-sm text-neutral-600 mt-2">{item.year ?? "—"}{item.rating ? ` · КП ${item.rating}` : ""}</div>
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        ...
        <form action={importMovieFromTmdb} className="vip-panel p-5 grid md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-bold mb-2 text-[#333]">Тип</label>
            <select name="type" className="w-full rounded-xl border border-[#ddd] bg-white text-[#222] h-12 px-4 outline-none focus:border-[#e50914]">
              <option value="MOVIE">Фильм</option>
              <option value="SERIES">Сериал</option>
              <option value="CARTOON">Мультфильм</option>
              <option value="ANIME">Аниме</option>
          ...
          <Field label="Alloha ID" name="allohaId" placeholder="можно добавить позже" className="md:col-span-2" />
          <button className="md:col-span-2 rounded-xl bg-[#e50914] text-white font-bold h-12" type="submit">Импортировать из TMDB</button>
        </form>
      </div>
    </div>
  );
...
function Field({ label, name, required, placeholder, defaultValue, className = "" }: { label: string; name: string; required?: boolean; placeholder?: string; defaultValue?: string; className?: string }) {
  return (
    <div className={className}>
      ...
      <input name={name} required={required} placeholder={placeholder} defaultValue={defaultValue} className="w-full rounded-xl border border-[#ddd] bg-white text-[#222] h-12 px-4 outline-none focus:border-[#e50914]" />
    </div>
  );
}
