...
export default async function BulkImportPage({ searchParams }: Props) {
  ...
  return (
    <div className="container py-6">
      ...
      <h1 className="text-3xl font-bold mt-3 mb-2">Массовый импорт</h1>
      ...
      <form action={bulkImportFromKinopoisk} className="vip-panel p-5 grid md:grid-cols-2 gap-4 max-w-3xl">
        <div className="md:col-span-2">
          <label className="block text-sm font-bold mb-2 text-white/80">Что импортировать</label>
          <select name="collection" className="w-full rounded-2xl border border-white/10 bg-[#0b1020] h-12 px-4 outline-none">
        ...
        <button className="md:col-span-2 rounded-2xl bg-gradient-to-r from-[#f7e2a9] via-[#c9a86a] to-[#8a6d3a] text-[#0b1020] font-bold h-12" type="submit">
...
function Field({ label, name, defaultValue, helper, className = "" }: { label: string; name: string; defaultValue?: string; helper?: string; className?: string }) {
  return (
    <div className={className}>
      ...
      <input name={name} defaultValue={defaultValue} className="w-full rounded-2xl border border-white/10 bg-white/[0.05] h-12 px-4 outline-none" />
      {helper ? <div className="text-xs text-white/45 mt-2">{helper}</div> : null}
    </div>
  );
}
...
function Result({ title, value }: { title: string; value: string }) {
  return (
    <div className="vip-soft-panel p-4">
      ...
      <div className="text-3xl font-black gold-text mt-1">{value}</div>
    </div>
  );
}
