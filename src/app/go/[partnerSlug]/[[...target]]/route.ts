...
export async function GET(request: Request, { params }: Props) {
  ...
  let redirectPath = `/collections/${partner.slug}`;
  ...
  if (partnerLink?.targetUrl) {
    ...
    const collection = hub ? await prisma.creatorCollection.findFirst({ where: { hubId: hub.id, slug: target[0], status: "PUBLISHED" } }) : null;
    if (collection) {
      redirectPath = `/collections/${partner.slug}/${collection.slug}`;
