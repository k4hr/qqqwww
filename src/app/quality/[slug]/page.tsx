...
export default async function QualityPage({ params }: Props) {
  ...
  return <div className="container py-6"><JsonLd data={{ "@context": "https://schema.org", "@type": "CollectionPage", name: `Фильмы в ${page.name} качестве`, mainEntity: { "@type": "ItemList", itemListElement: movies.map((movie, index) => ({ "@type": "ListItem", position: index + 1, name: movie.titleRu, url: siteUrl(watchPath(movie)) })) } }} /><section className="rf-catalog-intro mb-7"><h1 className="rf-page-title">Фильмы в {page.name} качестве</h1><p className="rf-copy mt-4 max-w-4xl">Настоящая коллекция доступных фильмов с отметкой качества {page.name}; без отдельных дублей под рекламные формулировки.</p></section><div className="movie-grid">{movies.map((movie) => <MovieCard key={movie.id} movie={movie} />)}</div></div>;
}
