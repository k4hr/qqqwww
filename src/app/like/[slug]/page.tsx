...
import { filmPath, likePath, similarPath, siteUrl } from "@/lib/seo-links";
...
export default async function LikePage({ params }: Props) {
  ...
  return <div className="container py-6">
    <JsonLd data={{ "@context": "https://schema.org", "@type": "CollectionPage", name: `Что посмотреть после ${movie.titleRu}`, url: siteUrl(likePath(movie)), mainEntity: { "@type": "ItemList", itemListElement: movies.map((item, index) => ({ "@type": "ListItem", position: index + 1, name: item.titleRu, url: siteUrl(filmPath(item)) })) } }} />
    <nav className="mb-5 text-sm text-[#85858f]"><Link href={filmPath(movie)}>{movie.titleRu}</Link> / Что посмотреть после</nav>
