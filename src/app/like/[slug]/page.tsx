...
export default async function LikePage({ params }: Props) {
  ...
  return <div className="container py-6">
    ...
    <section className="rf-catalog-intro"><h1 className="rf-page-title">Что посмотреть если понравился {movie.titleRu}</h1>{intro.map((text) => <p key={text} className="rf-copy mt-4 max-w-4xl">{text}</p>)}<div className="mt-5 flex gap-3"><Link href={similarPath(movie)} className="mf-btn">Фильмы похожие на {movie.titleRu}</Link></div></section>
    <section className="rf-section"><h2 className="rf-section-title mb-5">Продолжение киновечера</h2><div className="movie-grid">{movies.map((item) => <MovieCard key={item.id} movie={item} />)}</div></section>
  </div>;
}
