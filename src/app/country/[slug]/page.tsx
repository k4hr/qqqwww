...
export default async function CountryPage({ params }: Props) {
  ...
  return <div className="container py-6">
    ...
    <section className="rf-catalog-intro mb-7"><h1 className="rf-page-title">Фильмы {page.name}</h1><p className="rf-copy mt-4 max-w-4xl">В подборке собраны фильмы и сериалы производства {page.name}, доступные для просмотра на REDFILM.</p><p className="rf-copy mt-3 max-w-4xl">Используйте жанры и годы ниже, чтобы перейти к нужной части каталога без повторного поиска.</p></section>
    <div className="movie-grid">{movies.map((movie) => <MovieCard key={movie.id} movie={movie} />)}</div>
    <section className="rf-section border-t border-white/[.055] pt-7"><h2 className="rf-section-title">Навигация по фильмам {page.name}</h2><div className="mt-4 flex flex-wrap gap-2">{genres.map((genre) => <Link key={genre.slug} href={genrePath(genre)} className="mf-btn">{genre.name}</Link>)}{years.map((year) => <Link key={year} href={`/year/${year}`} className="mf-btn">{year} год</Link>)}</div><h3 className="mt-6 text-sm font-semibold text-white">Топ страны</h3><div className="mt-3 flex flex-wrap gap-2">{movies.slice(0, 6).map((movie) => <Link key={movie.id} href={watchPath(movie)} className="mf-pill min-h-11">{movie.titleRu}</Link>)}</div><h3 className="mt-6 text-sm font-semibold text-white">Новые фильмы</h3><div className="mt-3 flex flex-wrap gap-2">{newest.map((movie) => <Link key={movie.id} href={watchPath(movie)} className="mf-pill min-h-11">{movie.titleRu}</Link>)}</div></section>
