...
async function findPerson(slug: string) {
  ...
  const people = await prisma.person.findMany({ select: { id: true, nameRu: true, nameOriginal: true }, take: 2000 });
  return people.find((person) => isPublicPersonName(person.nameRu) && (normalizeSlug(person.nameRu) === slug || (person.nameOriginal && isPublicPersonName(person.nameOriginal) && normalizeSlug(person.nameOriginal) === slug))) ?? null;
}
