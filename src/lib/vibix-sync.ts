...
export async function saveVibixVideo(video: VibixVideo, targetMovieId?: string, options: SaveVibixVideoOptions = {}) {
  ...
  if (existing) {
    ...
    const updated = await prisma.movie.update({
      ...
      data: {
        ...
        durationSource: data.duration ? "VIBIX" : undefined,
        durationLastSyncAt: data.duration ? syncedAt : undefined,
  ...
  const created = await prisma.movie.create({
    data: {
      ...
      durationSource: data.duration ? "VIBIX" : null,
      durationLastSyncAt: data.duration ? syncedAt : null,
